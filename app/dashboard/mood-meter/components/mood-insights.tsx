  ...
  const insights = useMemo(() => {
    if (moodEntries.length < 3) {
      return [{
        ...
        description: 'Track your mood for a few more days to get personalized insights and recommendations.',
    ...
    if (sleepEntries.length >= 3) {
      ...
      if (goodSleepEntries.length > 0 && poorSleepEntries.length > 0) {
        const goodSleepMood = goodSleepEntries.reduce((sum, entry) => sum + entry.mood_score, 0) / goodSleepEntries.length;
        const poorSleepMood = poorSleepEntries.reduce((sum, entry) => sum + entry.mood_score, 0) / poorSleepEntries.length;
    ...
    if (stressEntries.length >= 3) {
      ...
      if (highStressEntries.length > 0 && lowStressEntries.length > 0) {
        const highStressMood = highStressEntries.reduce((sum, entry) => sum + entry.mood_score, 0) / highStressEntries.length;
        const lowStressMood = lowStressEntries.reduce((sum, entry) => sum + entry.mood_score, 0) / lowStressEntries.length;
    ...
    if (morningEntries.length >= 2 && eveningEntries.length >= 2) {
      const morningAvg = morningEntries.reduce((sum, entry) => sum + entry.mood_score, 0) / morningEntries.length;
      const eveningAvg = eveningEntries.reduce((sum, entry) => sum + entry.mood_score, 0) / eveningEntries.length;
    ...
    // Consistency check
    const moodVariance = moodEntries.reduce((sum, entry) => sum + Math.pow(entry.mood_score - avgMood, 2), 0) / moodEntries.length;
    if (moodVariance < 2) {
      insights.push({
        type: 'positive',
        icon: <CheckCircle className="h-4 w-4" />,
        title: 'Your mood is stable',
        description: 'You maintain consistent mood levels, which indicates good emotional regulation.',
      ...
      insights.push({
        ...
        description: 'Your mood varies significantly. This could indicate external stressors or lifestyle factors.',
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={moodScore >= 80 ? 'default' : moodScore >= 60 ? 'secondary' : 'outline'}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            AI-powered recommendations based on your mood data
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
